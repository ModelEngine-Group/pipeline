...
IF EXISTS (SELECT 1 FROM pg_sequences WHERE sequencename = 'store_tag_id_seq') THEN
    PERFORM setval('store_tag_id_seq', COALESCE((SELECT MAX(id) FROM store_tag), 1), true);
END IF;
end
$$;