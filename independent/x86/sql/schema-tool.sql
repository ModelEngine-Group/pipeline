...
IF EXISTS (SELECT 1 FROM pg_sequences WHERE sequencename = 'store_tool_id_seq') THEN
    PERFORM setval('store_tool_id_seq', COALESCE((SELECT MAX(id) FROM store_tool), 1), true);
END IF;
end
$$;